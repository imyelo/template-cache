...
var eachFile = function (basepath, action) {
  ...
  var nextLevel = function (subpath) {
    _.each(fs.readdirSync(path.join(basepath, subpath)), function (filename) {
      var nextpath;
      if (isDirectory(path.join(basepath, (nextpath = path.join(subpath, filename))))) {
        nextLevel(nextpath);
      } else {
        action(path.join(subpath, filename));
      }
    });
  };
  nextLevel('./');
